...
DEFAULT_MAX_LENGTH: int = (
    120  # Video length variable, edit this on your own risk. It should work, but it's not supported
)
...
class TTSEngine:
    ...
    Args:
        ...
        post_data             : The post data object that contains the posts to read.
    ...
    def __init__(
        ...
        self.post_id = re.sub(r"[^\w\s-]", "", post_data["thread_id"])
        self.path = path + self.post_id + "/mp3"
    ...
    def run(self) -> Tuple[int, int]:
        ...
        self.add_periods()
        self.call_tts("title", process_text(self.post_data["thread_title"]))
        # processed_text = ##self.post_data["thread_post"] != ""
        idx = 0

        if settings.config["settings"]["storymode"]:
            if settings.config["settings"]["storymodemethod"] == 0:
                if len(self.post_data["thread_post"]) > self.tts_module.max_chars:
                    self.split_post(self.post_data["thread_post"], "postaudio")
                else:
                    self.call_tts("postaudio", process_text(self.post_data["thread_post"]))
            elif settings.config["settings"]["storymodemethod"] == 1:
                for idx, text in track(enumerate(self.post_data["thread_post"])):
                    self.call_tts(f"postaudio-{idx}", process_text(text))

        else:
            for idx, comment in track(enumerate(self.post_data["comments"]), "Saving..."):
    ...
    def call_tts(self, filename: str, text: str):
        ...
        try:
            ...
            # Apply voice speed change if defined and != 1.0
            voice_speed = settings.config["settings"]["tts"].get("voice_speed", 1.0)
            if voice_speed and float(voice_speed) != 1.0:
                clip = clip.with_effects([MultiplySpeed(float(voice_speed))])
                # We need to overwrite the file with the new speed
                # Write to a temp file and replace to avoid conflicts
                temp_filepath = filepath + ".tmp.mp3"
                clip.write_audiofile(temp_filepath, logger=None)
                clip.close()
                os.replace(temp_filepath, filepath)
                # Reopen to get length correctly
                clip = AudioFileClip(filepath)
...
def process_text(text: str, clean: bool = True):
    ...
    # Không dịch nữa vì nội dung Threads đã là tiếng Việt
    # Nếu cần dịch từ ngôn ngữ khác, bỏ comment dòng dưới
    # lang = settings.config["threads"]["post_lang"]
    # if lang:
