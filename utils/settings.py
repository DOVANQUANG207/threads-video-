...
def check_toml(template_file, config_file) -> Tuple[bool, Dict]:
    ...
    config = None
    try:
        template = tomlkit.parse(Path(template_file).read_text(encoding="utf-8"))
    except Exception as error:
        console.print(f"[red bold]Encountered error when trying to to load {template_file}: {error}")
        return False
    try:
        config = tomlkit.parse(Path(config_file).read_text(encoding="utf-8"))
    except Exception:
        console.print(
...
"""
    ...
    Path(config_file).write_text(tomlkit.dumps(config), encoding="utf-8")
    return config
