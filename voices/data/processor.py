...
"""Training sample processor for GHVoice.
...
Used by ``voices.training.builder`` to build the data pipeline.

Contains two processor classes:
- ``GHVoiceSampleProcessor``: Full processor used for training.
- ``GHVoiceSimpleSampleProcessor``: Simplified processor (not used for training).
"""
...
class GHVoiceSimpleSampleProcessor:
    """
    Handles the logic of processing a raw sample into tensors
    (masking, tokenization, etc.).
    This is a simpler version that does not include language, instructions,
        or denoising prompts.
    We do not use it for training as GHVoiceSampleProcessor can cover this case.
