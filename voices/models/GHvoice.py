...
"""Core GHVoice model implementation.

Defines the ``GHVoice`` model class, generation config, and inference pipeline.
This is the main entry point for both inference and training:

- **Inference**: ``GHVoice.from_pretrained()`` loads the model, then
...
- **Training**: ``model.forward()`` computes the training loss; the model is
  built and used by ``voices.training.builder`` and ``voices.training.trainer``.
...
from voices.utils.audio import (
    cross_fade_chunks,
    fade_and_pad_audio,
    load_audio,
    remove_silence,
...
from voices.utils.lang_map import LANG_IDS, LANG_NAMES
from voices.utils.text import add_punctuation, chunk_text_punctuation
from voices.utils.voice_design import (
    _INSTRUCT_ALL_VALID,
    _INSTRUCT_EN_TO_ZH,
...
class GenerationTask:
    ...
    def get_indices(self, config: GHVoiceGenerationConfig, frame_rate: int):
...
@dataclass
class GHVoiceModelOutput(ModelOutput):
...
class GHVoiceConfig(PretrainedConfig):
...
class GHVoice(PreTrainedModel):
    _supports_flex_attn = True
    _supports_flash_attn_2 = True
    config_class = GHVoiceConfig

    def __init__(self, config: GHVoiceConfig, llm: Optional[PreTrainedModel] = None):
    ...
    def generate(
        ...
        generation_config: Optional[GHVoiceGenerationConfig] = None,
        **kwargs,
    ) -> list[np.ndarray]:
        ...
        if self.audio_tokenizer is None or self.text_tokenizer is None:
            raise RuntimeError(
                ...
                "loaded the model with GHVoice.from_pretrained()."
            )
        gen_config = (
            generation_config
            if generation_config is not None
            else GHVoiceGenerationConfig.from_dict(kwargs)
        )
    ...
    def _generate_chunked(
        self, task: GenerationTask, gen_config: GHVoiceGenerationConfig
    ...
    def _generate_iterative(
        ...
        Args:
            ...
            gen_config: A :class:`GHVoiceGenerationConfig` controlling
...
def _resolve_language(language: Optional[str]) -> Union[str, None]:
    from voices.utils.lang_map import LANG_IDS, LANG_NAME_TO_ID
...
AutoConfig.register("omnivoice", GHVoiceConfig)
AutoModel.register(GHVoiceConfig, GHVoice)
