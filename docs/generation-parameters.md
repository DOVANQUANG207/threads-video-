...
Parameters can be passed as keyword arguments to `model.generate(...)` or via the `GHVoiceGenerationConfig` dataclass. See below for the full list and which category each belongs to.

```python
# 1) Direct keyword arguments
audio = model.generate(text="Hello world", num_step=32, guidance_scale=2.0)

# 2) Via GHVoiceGenerationConfig dataclass
from GHvoice import GHVoiceGenerationConfig

config = GHVoiceGenerationConfig(num_step=32, guidance_scale=2.0)
