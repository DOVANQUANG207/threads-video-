...
python -m GHvoice.scripts.denoise_audio \
...
> The next step would be passing the generated `data.lst` file with `--input_manifest` to `GHvoice.scripts.extract_audio_tokens` for tokens extraction.
...
python -m GHvoice.scripts.extract_audio_tokens_add_noise \
    --input_jsonl data.jsonl \
