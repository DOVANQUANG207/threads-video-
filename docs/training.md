...
```json
{
    "resume_from_checkpoint": "exp/GHvoice/checkpoint-100000"
}
```

## Initializing from a Pretrained Model

To start training from a pretrained GHVoice checkpoint (for fine-tuning):

```json
{
    "init_from_checkpoint": "exp/GHvoice/checkpoint-100000"
}
```
...
```bash
tensorboard --logdir exp/GHvoice_emilia/tensorboard
```
