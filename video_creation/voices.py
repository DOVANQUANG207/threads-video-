...
from TTS.gh_voice import GHVoice
...
def save_text_to_mp3(post_data) -> Tuple[int, int]:
    """Saves text to MP3 files.

    Args:
        post_data (): Post data object
    ...
    if str(voice).casefold() in map(lambda _: _.casefold(), TTSProviders):
        text_to_mp3 = TTSEngine(get_case_insensitive_key_value(TTSProviders, voice), post_data)
    else:
        while True:
        ...
        text_to_mp3 = TTSEngine(get_case_insensitive_key_value(TTSProviders, choice), post_data)
